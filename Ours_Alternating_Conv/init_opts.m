...
% -------------------------------------------------------------------------
    ...
    opts.patch_size 	    = 16;       % patch size need to be devisable by scale
    opts.data_augmentation  = 0;

    %% dataset options
    opts.train_dataset          = {};
    opts.train_dataset{end+1}   = 'Train';
    opts.train_data_path	= 'Dataset/Stanford';
    opts.valid_dataset          = {};
    opts.valid_dataset{end+1}   = 'Valid';
    opts.valid_data_path	= 'Dataset/Stanford';
    opts.test_data_path		= 'Dataset/Stanford';
    ...
    opts.net_name = sprintf('Spatial_SR_x%d_depth%d_%s', ...
                            opts.scale, opts.depth, opts.loss);

    opts.model_name = sprintf('%s_%s_pw%d_lr%s_step%d_drop%s_min%s_batch_%s', ...
                            opts.net_name, ...
                            opts.data_name, opts.patch_size, ...
                            num2str(opts.lr), opts.lr_step, ...
                            num2str(opts.lr_drop), num2str(opts.lr_min), num2str(opts.batch_size));
