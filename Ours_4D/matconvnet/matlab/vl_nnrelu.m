...
%sav = gather(x);
%disp(size(x));
%sav = reshape(sav, [187, 270, 8, 8, 64]);
%sav = permute(sav, [1,3,2,4,5]);
%sav = reshape(sav, [187*8, 270*8, 64]);
%ind = floor(( size(dir('/media/data/Henry_Storage/LapSRN_4D/visual/'), 1) - 2 ) / 64 ) + 1;
%for kk = 1:64
%imwrite(sav(:,:,kk), ['/media/data/Henry_Storage/LapSRN_4D/visual/' num2str(ind) '_' num2str(kk) '.png']);
%end
