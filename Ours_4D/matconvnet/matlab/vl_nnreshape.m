function y = vl_nnreshape(x, dim, sp2an, dzdy)
...
if sp2an
    if nargin < 4 || isempty(dzdy)
        [h,w,c,n] = size(x);
        x_tmp = reshape(x, [h, w, c, dim*dim, n/dim/dim]);
        x_tmp = reshape(x_tmp, [h, w, c, dim, dim, n/dim/dim]);
        y = permute(x_tmp,[1,2,4,5,3,6]);
    else
        [h, w, dim1, dim2, c, n] = size(dzdy);
        der_tmp = permute(dzdy, [1,2,5,3,4,6]);
        der_tmp = reshape(der_tmp, [h, w, c, dim1*dim2, n]);
        y = reshape(der_tmp, [h,w,c,dim1*dim2*n]);
    end
else
    if nargin < 4 || isempty(dzdy)
        [h, w, dim1, dim2, c, n] = size(x);
        x_tmp = permute(x, [1,2,5,3,4,6]);
        x_tmp = reshape(x_tmp, [h, w, c, dim1*dim2, n]);
        y = reshape(x_tmp, [h,w,c,dim1*dim2*n]);
    else
        [h,w,c,n] = size(dzdy);
        der_tmp = reshape(dzdy,[h,w,c,dim*dim,n/dim/dim]);
        der_tmp = reshape(der_tmp,[h,w,c,dim,dim,n/dim/dim]);
        y = permute(der_tmp, [1,2,4,5,3,6]);
    end
end
